...
class IntaSS(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.min_ss_duration = (
            0.5  # Minimum duration of SS in seconds (set to 0.5 according to INTA)
        )
    ...
    def _load_from_source(self):
        ...
        for i, subject_id in enumerate(data_paths.keys()):
            ...
            if len(path_dict[KEY_FILE_EEG]) == 2:
                ...
                hypnogram_original = np.concatenate(
                    [hypnogram_original_1, states_between, hypnogram_original_2]
                )
                # Now marks
                marks_1 = self._read_marks(path_dict["%s_1" % KEY_FILE_MARKS][0])
                marks_2 = self._read_marks(path_dict["%s_1" % KEY_FILE_MARKS][1])
                marks_1 = utils.filter_stamps(
                    marks_1, 0, signal_1.size - 1
                )  # avoid runaway
            ...
            print(
                "Loaded ID %d (%s) (%02d/%02d ready). Time elapsed: %1.4f [s]"
                % (
                    subject_id,
                    self.get_name(subject_id),
                    i + 1,
                    n_data,
                    time.time() - start,
                )
    ...
    def _get_file_paths(self):
        ...
        for subject_id in self.all_ids:
            ...
            subject_recs.sort()
            path_eeg_file = tuple(
                [os.path.join(self.dataset_dir, PATH_REC, f) for f in subject_recs]
            )
            # Find states
            subject_states = [f for f in state_files if subject_name in f]
            subject_states.sort()
            path_states_file = tuple(
                [os.path.join(self.dataset_dir, PATH_STATES, f) for f in subject_states]
            )
            # Find spindles
            subject_spindles = [
                f for f in spindle_files_manual_fix if subject_name in f
            ]
            if len(subject_spindles) == 0:
                print(
                    "Subject %02d (%s): Spindle manual session not found, using automatic NewerWins fix instead."
                    % (subject_id, subject_name)
                )
                subject_spindles = [
                    f for f in spindle_files_auto_fix if subject_name in f
                ]
            else:
                print(
                    "Subject %02d (%s): Using spindle manual session."
                    % (subject_id, subject_name)
                )
    ...
    def _repair_stamps(self):
        ...
        for name in names_in_files:
            print("Fixing %s" % name)
            path_marks_file = os.path.abspath(
                os.path.join(inta_folder, PATH_MARKS, "original", "SS_%s.txt" % name)
            )
            path_eeg_file = os.path.abspath(
                os.path.join(inta_folder, PATH_REC, "%s.rec" % name)
            )
            ...
            print(
                "Originally: %d marks with valid=1, %d marks with valid=2"
            ...
            print(
                "After duration criterion: %d marks with valid=1, %d marks with valid=2"
                % (len(raw_marks_1), len(raw_marks_2))
            )
            ...
            path_new_marks_file = os.path.abspath(
                os.path.join(inta_folder, PATH_MARKS, filename_format % name)
            )
