...
class ModaSS(Dataset):
    ...
    def cv_split(self, n_folds, fold_id, seed=0, subject_ids=None):
        ...
        phases = np.asarray(
            [self.data[subject_id][KEY_PHASE] for subject_id in subject_ids]
        )
        n_blocks = np.asarray(
            [self.data[subject_id][KEY_N_BLOCKS] for subject_id in subject_ids]
        )
        # Groups of subjects
    ...
    def _get_data_path(self):
        return os.path.abspath(
            os.path.join(utils.PATH_DATA, PATH_MODA_RELATIVE, PATH_SEGMENTS)
        )
    ...
    def _load_from_source(self):
        ...
        for i, subject_id in enumerate(self.all_ids):
            ...
            print(
                "Loaded ID %s (%03d/%03d ready). Time elapsed: %1.4f [s]"
    ...
    def _prepare_signals(self, list_of_segments):
        ...
        crop_size = int(
            self.original_fs * (self.original_border_duration - target_border_duration)
        )
        ...
        if self.fs != self.original_fs:
            print(
                "Resampling from %d Hz to required %d Hz" % (self.original_fs, self.fs)
            )
            signal = utils.resample_signal(
                signal, fs_old=self.original_fs, fs_new=self.fs
            )
        else:
    ...
    def _prepare_labels(self, list_of_labels):
        ...
        binary_labels = np.clip(
            labels, a_min=0, a_max=1
        )  # borders will have a label of zero
        marks = utils.seq2stamp(binary_labels)
        marks_time = marks / self.original_fs  # sample to seconds
        # Transforms to sample-stamps
        marks = np.round(marks_time * self.fs).astype(
            np.int32
        )  # second to samples in target fs
