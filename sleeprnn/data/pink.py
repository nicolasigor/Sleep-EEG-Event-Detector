...
class Pink(Dataset):
    def __init__(self, params=None, load_checkpoint=False, verbose=True, **kwargs):
        ...
        self.signal_duration = (
            3600 + 2 * 20
        )  # 1 hour of useful signal + 1 page at borders
    ...
    def _load_from_source(self):
        ...
        for i, subject_id in enumerate(self.all_ids):
            ...
            hypnogram = (
                [self.unknown_id] + (n_pages - 2) * [self.n2_id] + [self.unknown_id]
            )
    ...
    def _get_profile_fn(self):
        pink_profile = np.load(
            os.path.join(utils.PATH_DATA, PATH_PINK_RELATIVE, "pink_profile.npy")
        )
    ...
    def _generate_signal(self, seed):
        ...
        y = np.fft.rfft(x)
        freq_gen = np.fft.rfftfreq(x.size, d=1.0 / self.fs)
