...
def generate_ensemble_from_probabilities(
    dict_of_proba, reference_feeder_dataset, skip_setting_threshold=False
):
    """
    dict_of_proba = {
    ...
    for subject_id in subject_ids:
        probabilities = (
            np.stack(dict_of_proba[subject_id], axis=0)
            .astype(np.float32)
            .mean(axis=0)
            .astype(np.float16)
        )
...
def generate_ensemble_from_stamps(
    ...
    for subject_id in subject_ids:
        ...
        subject_max_sample = downsampling_factor * (
            (subject_max_sample // downsampling_factor) + 10
        )
        probabilities = [
            utils.stamp2seq(single_stamp, 0, subject_max_sample - 1)
            .reshape(-1, downsampling_factor)
            .mean(axis=1)
            for single_stamp in stamps_list
        ]
...
def generate_ensemble_from_predicted_datasets(
    ...
    for subject_id in subject_ids:
        if use_probabilities:
            ...
            data_list = [
                pred.get_subject_stamps(subject_id) for pred in predicted_dataset_list
            ]
