...
class BaseModel(object):
    ...
    def __init__(
        ...
        with tf.variable_scope("inputs_ph"):
            ...
            self.feats_train_1_ph = tf.placeholder(
                tf.float32,
                shape=[None] + self.feat_train_shape,
                name="feats_train_1_ph",
            )
            self.labels_train_1_ph = tf.placeholder(
                tf.int8, shape=[None] + self.label_train_shape, name="labels_train_1_ph"
            )
            self.masks_train_1_ph = tf.placeholder(
                tf.int8, shape=[None] + self.label_train_shape, name="masks_train_1_ph"
            )
            ...
            self.labels_train_2_ph = tf.placeholder(
                tf.int8, shape=[None] + self.label_train_shape, name="labels_train_2_ph"
            )
            self.masks_train_2_ph = tf.placeholder(
                tf.int8, shape=[None] + self.label_train_shape, name="masks_train_2_ph"
            )

            self.feats_eval_ph = tf.placeholder(
                tf.float32, shape=[None] + self.feat_eval_shape, name="feats_eval_ph"
            )
            self.labels_eval_ph = tf.placeholder(
                tf.int8, shape=[None] + self.label_eval_shape, name="labels_eval_ph"
            )
            self.masks_eval_ph = tf.placeholder(
                tf.int8, shape=[None] + self.label_eval_shape, name="masks_eval_ph"
            )
        ...
        with tf.variable_scope("feeding"):
            ...
            self.iterator = feeding.get_global_iterator(
                self.handle_ph, iterators_list, name="iters"
            )
        ...
        self.train_step, self.reset_optimizer, self.grad_norm_summ = (
            self._optimizer_fn()
        )
        ...
        with tf.variable_scope("by_event_metrics"):
            self.eval_threshold = tf.placeholder(
                tf.float32, shape=[], name="threshold_ph"
            )
            self.eval_af1 = tf.placeholder(tf.float32, shape=[], name="af1_ph")
            self.eval_af1_half = tf.placeholder(
                tf.float32, shape=[], name="af1_half_ph"
            )
            self.eval_f1 = tf.placeholder(tf.float32, shape=[], name="f1_ph")
            self.eval_precision = tf.placeholder(
                tf.float32, shape=[], name="precision_ph"
            )
        ...
        # Summary writers
        self.train_writer = tf.summary.FileWriter(os.path.join(self.logdir, "train"))
        self.val_writer = tf.summary.FileWriter(os.path.join(self.logdir, "val"))
    ...
    def predict_dataset(
        ...
        border_size = int(
            np.round(self.params[pkeys.BORDER_DURATION] * self.params[pkeys.FS])
        )
        ...
        if time_reverse:  # Recover proper time ordering
            probabilities_list = [
                np.flip(single_p, axis=1) for single_p in probabilities_list
            ]
    ...
    def predict_proba_from_vector(self, x, with_augmented_page=False):
        ...
        y_batched = self.predict_proba(
            x_batched, with_augmented_page=with_augmented_page
        )

        y = pages2seq(y_batched, pages)

        original_samples_downsampled = (
            original_samples // self.params[pkeys.TOTAL_DOWNSAMPLING_FACTOR]
        )
    ...
    def predict_proba_with_list(self, x_list, verbose=False, with_augmented_page=False):
        ...
        for i, x in enumerate(x_list):
            if verbose:
                print(
                    "Predicting %d / %d ... " % (i + 1, len(x_list)), end="", flush=True
                )
            this_pred = self.predict_proba(x, with_augmented_page=with_augmented_page)
    ...
    def predict_tensor_at_samples_with_list(
        self, x_list, samples_list, tensor_name="last_hidden", verbose=False
    ):
        result_list = []
        ...
        for i in range(n):
            ...
            this_result = self.predict_tensor_at_samples(
                x_list[i], samples_list[i], tensor_name=tensor_name
            )
    ...
    def evaluate(self, x, y, m):
        ...
        for i in range(niters):
            eval_metrics = self.sess.run(
                ...
                feed_dict={self.training_ph: False, self.handle_ph: self.handle_eval},
            )
        ...
        feed_dict = {}
        for key in self.eval_metrics_dict:
            feed_dict.update({self.eval_metrics_dict[key]: mean_metrics[key]})
    ...
    def _init_iterator_train(
        self, x_train_1, y_train_1, m_train_1, x_train_2, y_train_2, m_train_2
    ):
    ...
    def _init_iterator_eval(self, x_eval, y_eval, m_eval):
