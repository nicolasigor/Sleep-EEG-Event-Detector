...
def lowpass_tf_batch(
    signals, fs, cutoff, filter_duration_ref=6, wave_expansion_factor=0.5
):
    ...
    lp_kernel = firwin(numtaps, cutoff=cutoff, window="hamming", fs=fs).astype(
        np.float32
    )
...
def bandpass_tf_batch(
    signals, fs, lowcut, highcut, filter_duration_ref=6, wave_expansion_factor=0.5
):
    new_signals = signals
...
def zscore_tf(signals, dispersion_mode=constants.DISPERSION_STD_ROBUST):
    ...
    if dispersion_mode == constants.DISPERSION_MADE:
        ...
        std_signals = tf.math.sqrt(
            tf.reduce_mean(sorted_values_valid**2, axis=1, keepdims=True)
        )
    else:
        raise ValueError()
...
def a7_layer_tf(
    ...
    with tf.variable_scope("a7_layer"):
        ...
        print(
            "Moving window: Using %1.2f s (%d samples)"
            % (window_duration, lp_filter_size)
        )
        print(
            "Moving window in relSigPow: Using %1.2f s (%d samples)"
        ...
        signal_sigma_squared = signal_sigma**2
        abs_sig_pow_raw_2 = moving_average_tf(
            signal_sigma_squared, lp_filter_size_relSigPow
        )
        signal_no_delta_squared = signal_no_delta**2
        abs_no_delta_pow_raw = moving_average_tf(
            signal_no_delta_squared, lp_filter_size_relSigPow
        )
        ...
        # sigma covariance
        sigma_centered = signal_sigma - tf.reduce_mean(
            signal_sigma, axis=1, keepdims=True
        )
        if remove_delta_in_cov:
            broad_centered = signal_no_delta - tf.reduce_mean(
                signal_no_delta, axis=1, keepdims=True
            )
        ...
        if return_raw_values:
            a7_parameters_raw = tf.stack(
                [abs_sig_pow_raw, rel_sig_pow_raw, sig_cov_raw, sig_corr_raw], axis=2
            )
...
def a7_layer_v2_tf(
    ...
    with tf.variable_scope("a7_dense_feats"):
        ...
        print(
            "Moving window: Using %1.2f s initial duration (%d samples)"
            % (window_duration, initial_lp_filter_size)
        )
        print("Z-score: Using '%s'" % zscore_dispersion_mode)
        print("Broad band low cutoff for relative power:", rel_power_broad_lowcut)
        print(
            "Broad band low cutoff for covariance/correlation:", covariance_broad_lowcut
        )

        signal_sigma = bandpass_tf_batch(
            signals, fs, sigma_frequencies[0], sigma_frequencies[1]
        )
        signal_rel_power_broad = bandpass_tf_batch(
            signals, fs, rel_power_broad_lowcut, None
        )
        signal_covariance_broad = bandpass_tf_batch(
            signals, fs, covariance_broad_lowcut, None
        )
        ...
        # Relative sigma power
        broad_power = apply_fir_filter_tf_batch(
            signal_rel_power_broad**2, lp_filter_kernel
        )
        ...
        broad_mean = apply_fir_filter_tf_batch(
            signal_covariance_broad, lp_filter_kernel
        )
        sigma_broad_mean_product = apply_fir_filter_tf_batch(
            signal_sigma * signal_covariance_broad, lp_filter_kernel
        )
        ...
        broad_squared = apply_fir_filter_tf_batch(
            signal_covariance_broad**2, lp_filter_kernel
        )
        ...
        for feat_name in feats_dict.keys():
            ...
            if transformations_dict[feat_name] is None:
                ...
                feat_signal = (
                    tf.nn.relu(feat_signal)
                    if feat_name in ["covariance", "correlation"]
                    else feat_signal
                )
                ...
                raise ValueError(
                    "Transformation %s not supported" % transformations_dict[feat_name]
                )
        ...
        features_stacked = tf.stack(
            [
                feats_dict["abs_power"],
                feats_dict["rel_power"],
                feats_dict["covariance"],
                feats_dict["correlation"],
            ],
            axis=2,
        )
