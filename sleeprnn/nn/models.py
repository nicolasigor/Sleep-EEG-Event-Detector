...
from sleeprnn.detection.metrics import (
    metric_vs_iou_macro_average,
    metric_vs_iou_micro_average,
)
from sleeprnn.detection.metrics import (
    average_metric_macro_average,
    average_metric_micro_average,
)
...
class WaveletBLSTM(BaseModel):
    ...
    def fit_without_validation(
        ...
        list_of_outputs = utils.shuffle_data_collection(
            [x_train, y_train, m_train], seed=0
        )
        ...
        x_train_1, y_train_1, m_train_1, x_train_2, y_train_2, m_train_2 = (
            self._split_train(x_train, y_train, m_train)
        )
        ...
        for it in range(1, total_iters + 1):
            ...
            if it % nstats == 0 or it == 1 or it == total_iters:
                ...
                print(
                    "It %6.0d/%d - %s - %s - E.T. %1.2fs (%1.2fs/100it)"
                    % (
                        it,
                        total_iters,
                        loss_print,
            ...
            if lr_criterion:
                ...
                print(
                    "    Learning rate update (%d). New value: %s"
        ...
        last_model = {KEY_ITER: iter_saved_model, KEY_LOSS: 0, KEY_F1_SCORE: 0}
    ...
    def fit(
        ...
        iter_per_epoch = int(
            iters_resolution * max(np.round(iter_per_epoch / iters_resolution), 1)
        )
        ...
        for it in range(1, niters + 1):
            ...
            if it % nstats == 0 or it == 1 or it == niters:
                ...
                self.train_writer.add_summary(train_summ, it)
                metric_msg += " - train loss %1.4f f1 %1.4f" % (
                    train_loss,
                ...
                if it % iter_per_epoch == 0 or it == 1 or it == niters:
                    ...
                    metric_msg += " - val loss %1.4f f1 %1.4f AF1 %1.4f (thr %1.2f)" % (
                        ...
                        byevent_val_metrics["af1"],
                        byevent_val_metrics["threshold"],
                    )
                    # Time passed
                    elapsed = time.time() - start_time
                    time_rate_per_100 = 100 * (elapsed - last_elapsed) / (it - last_it)
                    last_it = it
                    last_elapsed = elapsed
                    metric_msg += " - E.T. %1.2fs (%1.2fs/100it)" % (
                        elapsed,
                        time_rate_per_100,
                    )
                    print(metric_msg)
                    ...
                    if lr_update_criterion == constants.LOSS_CRITERION:
                        improvement_criterion = (
                            val_loss
                            < (1.0 - rel_tol_criterion) * model_criterion[KEY_LOSS]
                        )
                    else:
                        improvement_criterion = (
                            byevent_val_metrics["af1"]
                            > (1.0 + rel_tol_criterion) * model_criterion[KEY_AF1]
                        )
                    ...
                    if lr_criterion:
                        if self.lr_updates < self.params[pkeys.MAX_LR_UPDATES]:
                            ...
                            print(
                                "    Maximum number (%d) of learning rate "
    ...
    def evaluate_byevent(
        ...
        nonzero_iou_list = [
            iou_matching[iou_matching > 0] for iou_matching in iou_matching_list
        ]
        ...
        byevent_af1_half = average_metric_fn_dict[average_mode](
            val_events_list, val_detections_list
        )
    ...
    def _augmentation_fn(self, feat, label, mask):
        ...
        print(
            "indep uniform noise proba %s, intens %s"
            % (indep_unif_noise_proba, indep_unif_noise_intens)
        )
        print(
            "random waves proba %s, params %s"
            % (random_waves_proba, random_waves_params)
        )
        print(
            "random anti waves proba %s, params %s"
            % (random_anti_waves_proba, random_anti_waves_params)
        )
        if indep_unif_noise_proba > 0:
            print("Applying INDEPENDENT UNIFORM noise augmentation")
            feat = augmentations.independent_uniform_noise(
                feat, indep_unif_noise_proba, indep_unif_noise_intens
            )
        if random_anti_waves_proba > 0:
            ...
            feat = augmentations.random_anti_waves_wrapper(
                feat,
                label,
                random_anti_waves_proba,
                self.params[pkeys.FS],
    ...
    def _loss_fn(self):
        ...
        checks.check_valid_value(
            type_loss, "type_loss", [constants.MASKED_SOFT_FOCAL_LOSS]
        )
        loss, loss_summ = losses.masked_soft_focal_loss(
            ...
            self.params[pkeys.SOFT_FOCAL_GAMMA],
            self.params[pkeys.SOFT_FOCAL_EPSILON],
        )
    ...
    def _optimizer_fn(self):
        ...
        train_step, reset_optimizer_op, grad_norm_summ = optimizers.adam_optimizer_fn(
            self.loss, self.learning_rate, self.params[pkeys.CLIP_NORM]
        )
    ...
    def _batch_metrics_fn(self):
        with tf.variable_scope("batch_metrics"):
            ...
            precision, recall, f1_score = metrics.precision_recall_f1score(tp, fp, fn)
            ...
            batch_metrics_summ = tf.summary.merge([prec_summ, rec_summ, f1_summ])
    ...
    def _eval_metrics_fn(self):
        with tf.variable_scope("eval_metrics"):
            ...
            eval_metrics_summ = [self.loss_sum, self.batch_metrics_summ]
    ...
    def _split_train(self, x_train, y_train, m_train):
        ...
        print(
            "Pages with activity: %d (%1.2f %% of total)"
