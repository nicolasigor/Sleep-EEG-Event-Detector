...
def generic_optimizer_fn(optimizer, loss, clip_norm):
    ...
    if clip_norm is not None:
        gradients, _ = tf.clip_by_global_norm(
            gradients, clip_norm, use_norm=grad_norm, name="clipping"
        )
        ...
        grad_norm_summ = tf.summary.merge([grad_norm_summ, clipped_grad_norm_summ])
    else:
...
def adam_optimizer_fn(loss, learning_rate, clip_norm):
...
def adam_w_optimizer_fn(loss, learning_rate, weight_decay, clip_norm):
...
def sgd_optimizer_fn(loss, learning_rate, momentum, clip_norm, use_nesterov):
...
def rmsprop_optimizer_fn(loss, learning_rate, momentum, clip_norm):
