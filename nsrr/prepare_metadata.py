...
if __name__ == "__main__":
    ...
    for config in configs:
        ...
        metadata_file = os.path.join(
            DATASETS_PATH, config["dataset_name"], "datasets", config["filename"]
        )
        meta_df = pd.read_csv(metadata_file, low_memory=False)
        # subject id
        subject_ids = meta_df[config["subject_id"]].values
        subject_ids = [
            prepare_suffix(sub_id, config["dataset_name"]) for sub_id in subject_ids
        ]
        subject_ids = np.array(
            ["%s%s" % (config["prefix"], sub_id) for sub_id in subject_ids],
            dtype="<U40",
        )
        ...
        sexes_str = [
            config["sex_map"][sex_value] for sex_value in sexes.astype(np.int32)
        ]
        ...
        # Create new table
        table = {"subject_id": subject_ids, "age": ages, "sex": sexes_str}
    ...
    fname1 = os.path.join(
        DATASETS_PATH, "chat1", "datasets", "chat-baseline-metadata.csv"
    )
    fname2 = os.path.join(
        DATASETS_PATH, "chat1", "datasets", "chat-baseline-nonrandomized-metadata.csv"
    )
